...
Não compartilha código com o PGO 5.x das pastas `Back/` e `Front/` da raiz.
...
| Arquivo | O que faz |
|---|---|
| `Back/Esquema.gs` | O contrato: as 12 abas, os cabeçalhos e o tipo de cada coluna |
| `Back/Planilha.gs` | A porta única para o Sheets: vínculo por cabeçalho e formato antes da gravação |
| `Back/Sequencia.gs` | O Id decimal de 10 casas, que nunca anda para trás |
| `Back/Instalador.gs` | Cria as 12 abas numa planilha vazia e semeia o que é editável |

## Instalar

1. Crie uma planilha **nova e vazia** e um projeto do Apps Script vinculado.
2. Copie os quatro `.gs` de `Back/`.
...
31 testes. O critério é **5 execuções seguidas sem falha**.

Os testes rodam contra um Google Planilhas falso que **converte valores igual
ao de verdade** (`Testes/sandbox.js`): numa célula de formato Geral,
...
|---|---|
| Alguém digitou linhas direto na planilha | `seqNormalizarBase_('BASE_MESA')` carimba os Ids que faltam |
| Uma coluna foi renomeada na planilha | `verificarEstruturaRECC()` mostra o que sumiu e o que apareceu |
| Precisa de uma coluna nova | `plAdicionarColuna_(aba, cabeçalho, tipo)` — cria no fim e registra o tipo em `CAMPOS` |
