...
function getBootstrapData() {
  ...
  // v4.6: mapa aninhado Produto → Categoria → [Subcategorias]. A chave por
  // produto evita ambiguidade quando o mesmo nome de categoria existe em
  // produtos diferentes (ex.: "Cobrança indevida" no cartão e na conta). O
  // frontend usa o produto e a categoria já selecionados para resolver a
  // lista dependente sem novas idas ao servidor.
  const subcategoriasPorCategoria = {};
  subcategorias.forEach(function(sub) {
    const categoria = categoriaPorId[String(sub.CategoriaId || '')];
    if (!categoria) return; // subcategoria órfã (categoria removida): ignorada.
    const produto = produtoPorId[String(categoria.ProdutoId || '')];
    const nomeProduto = produto ? produto.Nome : 'Geral';
    const nomeCategoria = String(categoria.Nome || '');
    if (!subcategoriasPorCategoria[nomeProduto]) subcategoriasPorCategoria[nomeProduto] = {};
    if (!subcategoriasPorCategoria[nomeProduto][nomeCategoria]) {
      subcategoriasPorCategoria[nomeProduto][nomeCategoria] = [];
    }
    subcategoriasPorCategoria[nomeProduto][nomeCategoria].push(String(sub.Nome || ''));
  });
  ...
  return {
    ...
    dropdownData: {
      ...
      subcategorias: pluck_(subcategorias, 'Nome'),
...
function preserveHiddenFields_(input, oldRecord) {
  ...
  const columnByField = {
    ...
    subcategoria: 'Subcategoria', canal: 'Canal', observacoes: 'Observacoes'
  };
  const hiddenExtras = {};
...
function salvarConfiguracao(entidade, dados, id) {
  ...
  // v4.6: toda subcategoria pertence obrigatoriamente a uma categoria.
  if (meta.key === 'subcategorias') {
    if (!record.CategoriaId) throw new Error('Selecione a categoria da subcategoria.');
    if (!getById(CONFIG.SHEET_NAMES.CATEGORIAS, record.CategoriaId)) {
      throw new Error('Categoria informada para a subcategoria não existe.');
    }
  }
...
function excluirConfiguracao(entidade, id) {
  ...
  if (meta.key === 'camposFormulario') {
    ...
    // v4.6: cascata — remove também as subcategorias filhas, evitando
    // registros órfãos na aba Subcategorias.
    removeSubcategoriasDaCategoria_(safeId);
    remove(meta.sheet, safeId);
  } else if (meta.key === 'subcategorias') {
    // v4.6: exclusão DEFINITIVA de subcategorias (linha removida da aba).
    remove(meta.sheet, safeId);
...
function getConfigurationEntities_() {
  return {
    ...
    // v4.6: subcategorias (hierarquia Produto → Categoria → Subcategoria).
    subcategorias: {
...
/**
 * v4.6: remove todas as subcategorias vinculadas a uma categoria. Usado na
 * exclusão em cascata de categorias, para não deixar subcategorias órfãs
 * (que ficariam invisíveis no formulário por não resolverem o produto pai).
 * @param {string} categoriaId - Id da categoria removida.
 */
function removeSubcategoriasDaCategoria_(categoriaId) {
  const alvo = String(categoriaId || '');
  if (!alvo) return;
  getAll(CONFIG.SHEET_NAMES.SUBCATEGORIAS).forEach(function(sub) {
    if (String(sub.CategoriaId || '') === alvo) {
      remove(CONFIG.SHEET_NAMES.SUBCATEGORIAS, sub.Id);
    }
  });
}
