...
const CONFIG = {
  ...
  SHEET_NAMES: {
    ...
    // v4.6: subcategorias vinculadas a uma categoria (hierarquia
    // Produto → Categoria → Subcategoria), administráveis pela tela de
    // Configurações. A subcategoria escolhida é gravada na coluna
    // "Subcategoria" de cada atendimento.
...
const COLUMNS = {
  ATENDIMENTOS: [
    ...
    'Produto',
    'Categoria',
    'Subcategoria',      // v4.6: nível filho de Categoria (hierarquia)
  ...
  // v4.6: cada subcategoria pertence a uma categoria (CategoriaId), que por
  // sua vez pertence a um produto — formando a hierarquia
...
const DEFAULT_CONFIG_CAMPOS = [
  ...
  // v4.6: subcategoria (nível filho de categoria). Aparece após a Categoria.
  { Id: 'FC010', Campo: 'subcategoria', Rotulo: 'Subcategoria',    Tipo: 'select',   Exibir: true,  Obrigatorio: false, Ordem: 7, Base: true,  Bloqueado: false },
  { Id: 'FC007', Campo: 'canal',        Rotulo: 'Canal',           Tipo: 'select',   Exibir: true,  Obrigatorio: true,  Ordem: 8, Base: true,  Bloqueado: true },
  { Id: 'FC008', Campo: 'observacoes',  Rotulo: 'Observações',     Tipo: 'textarea', Exibir: true,  Obrigatorio: false, Ordem: 9, Base: true,  Bloqueado: false },
  { Id: 'FC009', Campo: 'agencia',      Rotulo: 'Agência',         Tipo: 'text',     Exibir: false, Obrigatorio: false, Ordem: 10, Base: false, Bloqueado: false }
];
...
/**
 * Subcategorias vinculadas às categorias (campo "CategoriaId"), formando a
 * hierarquia Produto → Categoria → Subcategoria. v4.6 — política de não
 * sobrescrita: gravadas APENAS quando a aba "Subcategorias" está vazia
 * (primeira criação). Depois disso, o ADM administra tudo pela tela de
 * Configurações, sem alteração de código.
 */
const DEFAULT_SUBCATEGORIAS = [
  // Cartão de Crédito → Fatura (CT006)
  { Id: 'SC001', CategoriaId: 'CT006', Nome: 'Parcelamento de fatura', Descricao: 'Parcelamento do valor da fatura',        Ativo: true, Ordem: 1 },
  { Id: 'SC002', CategoriaId: 'CT006', Nome: 'Segunda via',            Descricao: 'Emissão de segunda via da fatura',       Ativo: true, Ordem: 2 },
  { Id: 'SC003', CategoriaId: 'CT006', Nome: 'Encargos',              Descricao: 'Juros e encargos da fatura',             Ativo: true, Ordem: 3 },
  // Cartão de Crédito → Limite de crédito (CT004)
  { Id: 'SC004', CategoriaId: 'CT004', Nome: 'Aumento de limite',      Descricao: 'Solicitação de aumento de limite',       Ativo: true, Ordem: 1 },
  { Id: 'SC005', CategoriaId: 'CT004', Nome: 'Redução de limite',      Descricao: 'Solicitação de redução de limite',       Ativo: true, Ordem: 2 },
  // Conta Digital → Transferência/Pix (CT009)
  { Id: 'SC006', CategoriaId: 'CT009', Nome: 'Pix agendado',           Descricao: 'Dúvidas ou problemas com Pix agendado',  Ativo: true, Ordem: 1 },
  { Id: 'SC007', CategoriaId: 'CT009', Nome: 'Pix não recebido',       Descricao: 'Pix enviado e não recebido pelo destino', Ativo: true, Ordem: 2 },
  { Id: 'SC008', CategoriaId: 'CT009', Nome: 'TED/DOC',                Descricao: 'Transferências via TED ou DOC',          Ativo: true, Ordem: 3 }
];
