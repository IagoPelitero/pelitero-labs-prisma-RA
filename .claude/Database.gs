...
function initializeSheets() {
  try {
    ...
    const sheetsConfig = [
      ...
      { name: CONFIG.SHEET_NAMES.SUBCATEGORIAS,  columns: COLUMNS.SUBCATEGORIAS, defaults: DEFAULT_SUBCATEGORIAS }
    ];
    ...
    ensureSubcategoriaField_(ss); // v4.6: injeta o campo "subcategoria" na ConfigCampos existente.
...
/**
 * Migração v4.6 — garante que o campo "subcategoria" exista na aba
 * ConfigCampos das instalações já em produção. Em bases novas, o campo já
 * vem em DEFAULT_CONFIG_CAMPOS; aqui tratamos apenas quem já tinha a
 * ConfigCampos populada antes desta versão. Idempotente: se o campo já
 * existir, não faz nada. A ordem é calculada para posicionar a
 * Subcategoria logo após a Categoria, sem reescrever os demais campos
 * (preserva qualquer reordenação feita pelo ADM).
 * @param {Spreadsheet} ss - Planilha do sistema.
 */
function ensureSubcategoriaField_(ss) {
  const sheet = ss.getSheetByName(CONFIG.SHEET_NAMES.CONFIG_CAMPOS);
  if (!sheet) return;
  const columns = COLUMNS.CONFIG_CAMPOS;
  const lastRow = sheet.getLastRow();
  if (lastRow < 1) return; // aba vazia: o seed padrão já inclui a subcategoria.

  const values = sheet.getRange(1, 1, lastRow, columns.length).getValues();
  const headers = values[0].map(function(value) { return String(value); });
  const campoIdx = headers.indexOf('Campo');
  const ordemIdx = headers.indexOf('Ordem');
  const idIdx = headers.indexOf('Id');
  if (campoIdx === -1) return;

  let jaExiste = false;
  let ordemCategoria = 6;
  const idsUsados = {};
  for (let i = 1; i < values.length; i++) {
    const campo = String(values[i][campoIdx] || '');
    if (campo === 'subcategoria') jaExiste = true;
    if (campo === 'categoria' && ordemIdx !== -1) {
      const n = Number(values[i][ordemIdx]);
      if (!isNaN(n)) ordemCategoria = n;
    }
    if (idIdx !== -1) idsUsados[String(values[i][idIdx] || '')] = true;
  }
  if (jaExiste) return;

  const novo = {
    Id: idsUsados['FC010'] ? generateId('FC') : 'FC010',
    Campo: 'subcategoria',
    Rotulo: 'Subcategoria',
    Tipo: 'select',
    Exibir: true,
    Obrigatorio: false,
    // Ordem fracionária posiciona a Subcategoria imediatamente após a
    // Categoria na ordenação do formulário, sem tocar nas demais linhas.
    Ordem: ordemCategoria + 0.5,
    Base: true,
    Bloqueado: false
  };
  sheet.appendRow(toRowArray(novo, columns));
  Logger.log('Campo "subcategoria" adicionado à ConfigCampos (migração v4.6).');
}
